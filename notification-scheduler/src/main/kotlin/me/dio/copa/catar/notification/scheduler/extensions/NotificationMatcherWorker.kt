...
class NotificationMatcherWorker(
    ...
    companion object {
        ...
        fun startMatchesNotFound(context: Context) {
            val inputData = workDataOf(
                NOTIFICATION_TITLE_KEY to "Nenhuma partida encontrada",
                NOTIFICATION_CONTENT_KEY to "Não foi possível encontrar nenhuma partida."
            )
            ...
            WorkManager.getInstance(context)
                .enqueueUniqueWork(
                    ...
                    createRequest(Duration.ofMinutes(1), inputData)
                )
        }


        fun startUnexpected(context: Context) {
            val inputData = workDataOf(
                NOTIFICATION_TITLE_KEY to "Erro Inesperado",
                NOTIFICATION_CONTENT_KEY to "Ocorreu um erro inesperado."
            )
